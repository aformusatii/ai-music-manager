...
- Direct YouTube downloads are now a first-class workflow:
  - Shared processing lives in `backend/src/services/youtubeDownloadProcessor.js`, which handles yt-dlp metadata extraction, filename sanitizing, LevelDB upserts (including `source` = `youtube_direct`), and download status updates for both Spotify-triggered jobs and direct submissions.
  - New API route `POST /api/youtube/direct-download` (`backend/src/routes/youtube.js`) accepts a YouTube video or playlist URL, classifies/sanitizes it via yt-dlp, dedupes tracks by `youtubeVideoId`, enqueues download jobs, and reports any playlist entries that were skipped (either failures or because they exceeded the cap).
  - `backend/src/db/tracks.js` now stores a `source` field, builds deterministic IDs for YouTube tracks (`yt_<videoId>`), exposes `findTrackByYoutubeVideoId`, and supports filtering by `source` from the API/UI.
  - The `DownloadManager` delegates all actual yt-dlp work to the processor so both Spotify and manual URLs follow the identical pipeline (metadata collection + file storage), while still resolving AI video IDs when needed.
  - Frontend has a dedicated Vue view (`frontend/src/views/DirectYoutubeDownloadView.vue`), route entry, nav link, and API client helper to schedule these jobs; the Local Tracks page shows a new Source column/filter so users can distinguish Spotify vs YouTube entries.
- Playlist/Mix imports are capped via `YOUTUBE_DIRECT_MAX_ITEMS` (`appConfig.youtube.directDownloadMaxItems`, default 50) to avoid enqueueing hundreds of jobs at once; the playlist response returns `processedEntries`/`skippedEntries` so the UI can communicate the limit. Increase this env var if bulk imports are desired.
